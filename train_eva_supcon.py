...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        """Compute loss for model features. If both `labels` and `mask` are None, it degenerates to unsupervised contrastive loss."""
        device = features.device

        # Ensure features is 2D (batch_size, num_features)
        if len(features.shape) == 4:  # In case features are (batch_size, channels, height, width)
        ...
        # We expect features to be of shape (batch_size, num_features)
        batch_size = features.shape[0]  # Number of samples in the batch
        contrast_feature = features  # No need to unbind, directly use features
        ...
        # Mask out self-contrast cases (i.e., the diagonal elements)
...
class Trainer:
    def __init__(
        ...
        self.contrastive_loss_fn = SupConLoss(temperature=0.07)  # Supervised Contrastive Loss
        self.epochs = epochs
    ...
    def train_epoch(self) -> float:
        ...
        for images, targets in progress_bar:
            ...
            ce_loss = self.loss_fn(outputs, targets)
            
            # Calculate supervised contrastive loss
            contrastive_loss = self.contrastive_loss_fn(features, labels=targets)
